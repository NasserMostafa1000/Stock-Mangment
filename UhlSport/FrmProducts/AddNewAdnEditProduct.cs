...
namespace FaisalSport
{
    public partial class AddNewAdnEditProduct : Form
    {
      ...
      private void GenrateParcode()
        {
            if (string.IsNullOrWhiteSpace(txSellingPrice.Text))
            {
                MessageBox.Show("من فضلك أدخل رقماً في مربع النص.");
                return;
            }

            try
            {
                // إنشاء باركود من الرقم
                ...
                // تحويل الرقم إلى صورة الباركود
                Bitmap barcodeBitmap = barcodeWriter.Write(txSellingPrice.Text);

                // عرض الباركود في PictureBox
                ProdutPicture.Image = barcodeBitmap;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"حدث خطأ أثناء إنشاء الباركود: {ex.Message}");
            }
        }
